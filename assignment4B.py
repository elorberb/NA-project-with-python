...
class Assignment4:
    ...
    def area(self, contour: callable, maxerr=0.001)->np.float32:
        ...
        result = 0
        n = 10 #first sample size
        points = contour(n)
        area = 0
        current_relative_err = maxerr+1
        while current_relative_err > maxerr:
            last_result = result
            for i in range(1,len(points)):
                area += (points[i][1] + points[i-1][1]) * (points[i-1][0] - points[i][0]) #trapezoid formula
            result = area / 2
            current_relative_err = np.abs((last_result - result)/result)
            n*=2 #increasing sample size
            area = 0
            points = contour(n)
